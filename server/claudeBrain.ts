...
function buildPrompt(input: BrainTaskInput): string {
  ...
  if (input.task === "episode-script" && input.episodeContext) {
    ...
    if (ep.storyFramework) {
      parts.push(`\nSHARED BOOK + VIDEO STORY FRAMEWORK:\n${ep.storyFramework}`);
    }
    parts.push(`\nSTORY BEATS:`);
    ...
    parts.push(
      [
        "\nHARD ALIGNMENT INSTRUCTIONS:",
        "Preserve the episode hook, conflict, reveal, escalation, cliffhanger, character list, island, and location exactly as the source spine.",
        "Do not invent new unlocks, face reveals, locations, relationships, or lore that conflict with the shared framework.",
        "Every scene must map cleanly into video beats without changing plot meaning.",
        "Carry forward book-builder continuity: character appearance, costume, props, powers, panel language, and latest continuity notes.",
        "If continuity is missing, stay conservative and use only the provided episode spine plus non-negotiable canon rules.",
      ].join("\n"),
    );
    parts.push(
      `\nDeliver: Full production script with The Web-Teller narration, scene directions, dialogue beats, and social captions. Runtime target: 75 seconds. Format: 9:16 vertical micro-episode. Every scene must align with the shared book/video framework and must be convertible into video prompts without changing canon.`,
    );
    ...
    parts.push(`BRIEF: ${input.brief}`);
    if (input.storyFramework) {
      parts.push(`OFFICIAL STORY ALIGNMENT PACK:\n${input.storyFramework}`);
    }
    parts.push(
      `Output a single optimized image-to-video or text-to-video prompt that preserves the episode/book continuity, character appearance locks (rounded chin, full lips, broad nose, Caribbean features), 2.5D anime style, action, camera angle, mood, lighting, and rhythm glyph effects if applicable. Under 300 words.`,
    );
