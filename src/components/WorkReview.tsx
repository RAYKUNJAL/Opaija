...
function getReviewTabFromHash(): "jobs" | "scripts" | "video" {
  const hash = window.location.hash.replace(/^#/, "").toLowerCase();
  if (hash.includes("video")) return "video";
  if (hash.includes("script")) return "scripts";
  return "jobs";
}
...
export function WorkReview({ token }: { token: string | null }) {
  const [jobs, setJobs] = useState<JobRecord[]>([]);
  const [queue, setQueue] = useState<QueueData | null>(null);
  const [loading, setLoading] = useState(true);
  const [tab, setTab] = useState<"jobs" | "scripts" | "video">(() => getReviewTabFromHash());
  ...
  async function load() {
    ...
    const [j, q] = await Promise.allSettled([
      fetch(apiUrl("/api/jobs?limit=100"), { headers }).then((r) => r.json()),
      fetch(apiUrl("/api/episodes"), { headers }).then((r) => r.json()),
    ]);
    if (j.status === "fulfilled") {
      const jobsData = j.value;
      setJobs(Array.isArray(jobsData) ? (jobsData as JobRecord[]) : []);
    }
    if (q.status === "fulfilled") {
      const queueData = q.value;
      setQueue(
        queueData && typeof queueData === "object" && Array.isArray((queueData as QueueData).episodes)
  ...
  async function pollJob(id: string) {
    try {
      const headers: Record<string, string> = token ? { "x-admin-session": token } : {};
      await fetch(apiUrl(`/api/jobs/${id}/poll`), { method: "POST", headers });
      await load();
    } catch {
      /* ignore */
    }
  ...
  async function pollActiveJobs() {
    ...
    await Promise.all(active.map((job) => pollJob(job.id)));
  }
  ...
  useEffect(() => {
    const syncTab = () => setTab(getReviewTabFromHash());
            ...
            {videoJobs.length === 0 ? (
              <div className="empty-log">
                ...
                <p className="dim">
                  ...
                  sub-tab, fill in a prompt, and click "Submit Video Render".
                </p>
              </div>
            ) : (
